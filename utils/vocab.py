...
class Vocab():
    ...
    def from_train(self, filepath, min_freq=1):
        with open(filepath, 'r', encoding='utf-8') as f:
            trains = json.load(f, object_pairs_hook=collections.OrderedDict)
        word_freq = {}
        for data in trains:
            for utt in data:
...
class LabelVocab():
    ...
    def from_filepath(self, root):
        ontology = json.load(open(os.path.join(root, 'ontology.json'), 'r', encoding='utf-8'), object_pairs_hook=collections.OrderedDict)
        acts = ontology['acts']
